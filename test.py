...
print(1, ls.users[user_id]['field'], ls.users[user_id]['search_data'], len(ls.users[user_id]['listing_id'])) # количество записей

ls.create_id_list(user_id)
print(2, ls.users[user_id]['field'], ls.users[user_id]['search_data'], len(ls.users[user_id]['listing_id'])) # количество записей

ls.search_parameters(user_id, field='autor')
ls.create_id_list(user_id)
print(3, ls.users[user_id]['field'], ls.users[user_id]['search_data'], len(ls.users[user_id]['listing_id'])) # количество записей
...
ls.search_parameters(user_id, field='content', search_data='Сло"во')
ls.create_id_list(user_id)
